      ...
      {showFilters && (
        <div className="absolute top-11 left-[300px] w-80 bg-white border border-slate-200 rounded-xl shadow-xl z-20 p-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="space-y-3">
            ...
            <div><label className="text-xs font-semibold text-slate-600 block mb-1">Status</label><select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })} className="w-full h-8 px-2 text-sm border rounded"><option value="">All</option><option value="Upcoming">Upcoming</option><option value="Completed">Completed</option><option value="Paused">Paused</option><option value="Cancelled">Cancelled</option></select></div>
